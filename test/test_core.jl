...
@testset "Dislocation Creep OPX" begin
    for T in (Float64, Float32)
        atol_ori = T == Float32 ? 1e-4 : 1e-12
        atol_frac = T == Float32 ? 1e-6 : 1e-15

        @testset "shear_dudz T=$T" begin
            for θ in range(0, 2π, length=360)
                R = rotation_from_rotvec([0.0, θ, 0.0])
                orientations = Array{T}(reshape(R, 1, 3, 3))
                ...
                @test isapprox(orientations_diff[1,:,:], target, atol=atol_ori)
                @test isapprox(sum(fractions_diff), 0.0, atol=atol_frac)
            end
        end

        @testset "shear_dvdx T=$T" begin
            for θ in range(0, 2π, length=361)
                ...
                ori_s = SMatrix{3,3,T,9}(R')  # transpose as orientation
                slip_rates = SVector{4,T}(0, 0, 0, 0)
                deformation_rate = _get_deformation_rate(enstatite, ori_s, slip_rates)
                @test isapprox(Array(deformation_rate), zeros(3,3), atol=atol_frac)
...
@testset "Dislocation Creep Olivine A" begin
    for T in (Float64, Float32)
        ...
        @testset "shear_dvdx_slip_010_100 T=$T" begin
            L = T.([0 0 0; 2 0 0; 0 0 0])
            sr = T.([0 1 0; 1 0 0; 0 0 0])
            ...
            for θ in range(0, 2π, length=3600)
                ...
                crss = get_crss(olivine, olivine_A, T)
                sr_s = SMatrix{3,3,T,9}(sr)
                ori_s = SMatrix{3,3,T,9}(R)
                slip_invariants = _get_slip_invariants(sr_s, ori_s)
                ratios = SVector{4,T}(abs(slip_invariants[1]/crss[1]),
        ...
        @testset "shear_dudz_slip_001_100 T=$T" begin
            L = T.([0 0 2; 0 0 0; 0 0 0])
            sr = T.([0 0 1; 0 0 0; 1 0 0])
